...
#
# Runs a fixed set of SMALL cases on BOTH kernels (atmospheric + ballistic),
# and for each checks:
#   * mass conservation        (|Total mass - 1| < MASS_TOL)   — physics invariant
#   * numerical correctness    atmos: rel Frobenius vs a committed golden < FROB_TOL
#                              ballistic: rel L2 (density) vs the MC reference < MC_TOL
...
#
# Usage:
#   bash sweeps/bench.sh                 # run all cases, compare to golden / MC
#   bash sweeps/bench.sh --make-golden   # (re)generate the atmospheric golden files
#
# Quick (login-runnable, ~2-4 min total). Memory here is the small-case footprint;
...
MC=${MC:-$ROOT/../Low-rank-Monte-Carlo/Low-rank-Monte-Carlo}
...
FROB_TOL=${FROB_TOL:-1e-6}     # atmos vs golden (parallel FP noise ~1e-8 << this)
MC_TOL=${MC_TOL:-0.01}         # ballistic rel L2 (density) vs MC (<1%)
...
"bal_t10      ballistic   monodiag 10    8192     256   128       1e-8    1e-8    4   mc:conc_ballistic_t10.dat"
"bal_t20      ballistic   monodiag 20    262144   256   256       1e-7    1e-8    4   mc:conc_ballistic_t20.dat"
)
...
if [ "${1:-}" = "--make-golden" ]; then
    echo "Generating atmospheric golden references..."
    for c in "${CASES[@]}"; do
...
for c in "${CASES[@]}"; do
    ...
    if [ "$check" = "golden" ]; then
        ...
        mcf="$MC/${check#mc:}"
        rl=$(python3 "$ROOT/sweeps/mc_compare.py" "$mcf" "$outf" 2>/dev/null | awk -F'= ' '/rel L2 .density/{print $2}')
        detail="relL2=${rl:-NA}"
        corrok=$(awk -v r="${rl:-9}" -v tol="$MC_TOL" 'BEGIN{print (r+0<tol)?1:0}')
    fi
